...
def getWordScore(word, n):
    ...
    total_points = 0
    for i in word:
        total_points += SCRABBLE_LETTER_VALUES[i]

    if n == len(word):
        return ((total_points * n) + 50)
...
def isValidWord(word, hand, wordList):
    ...
    if word in wordList:
        for i in word:
            if i not in new_hand.keys() or new_hand.get(i,0) < 1:
                return False
            elif i in new_hand.keys():
    ...
    while (calculateHandlen(hand) > 0):
        ...
        # Otherwise (the input is not a single period):
            ...
            # Otherwise (the word is valid):
                ...
                the_word_score = getWordScore(userInput,n)
                total_points += the_word_score
                print(userInput + " earned "+ str(the_word_score) + " points." + " Total: "+ str(total_points)+ " points")
